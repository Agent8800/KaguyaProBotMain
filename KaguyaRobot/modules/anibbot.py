...
from KaguyaRobot import BOT_USERNAME as BOT_NAME, TRIGGERS as trg, DEV_USERS as OWNER, pbot as anibot, DOWN_PATH, EVENT_LOGS as LOG_CHANNEL_ID
from KaguyaRobot.utils.db import get_collection
from KaguyaRobot.utils.helper import (
...
from KaguyaRobot.utils.data_parser import (
...
from .anibot_anilist import auth_link_cmd, code_cmd, logout_cmd
...
async def start_(client: anibot, message: Message, mdata: dict):
    ...
    if gid==user:
        if not (user in OWNER) and not (await USERS.find_one({"id": user})):
            ...
            await clog("kAGUYA", f"New User started bot\n\n[{usertitle}](tg://user?id={user})\nID: `{user}`", "NEW_USER")
